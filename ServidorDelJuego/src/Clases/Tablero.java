...
public class Tablero {
    ...
    public void setJugada(String jugada){
        ...
        if(jugada.contains(" al ")){
            ...
            int deMano=0;
            if(jugada.contains(Constantes.JUGADA_DE_MANO))deMano=5;
            if(jugada.contains(Constantes.JUGADA_ESCALERA))setEscalera(20+deMano);
            if(jugada.contains(Constantes.JUGADA_FULL))setFull(30+deMano);
            if(jugada.contains(Constantes.JUGADA_POQUER))setPoquer(40+deMano);
            if(jugada.contains(Constantes.JUGADA_GRANDE))setGrande(50);
            if(jugada.contains(Constantes.JUGADA_DORMIDA))llenarTodoAlMaximo();
        }else if(jugada.contains(" los ")){
                ...
                if(jugada.contains(Constantes.JUGADA_ESCALERA))setEscalera(-1);
                if(jugada.contains(Constantes.JUGADA_FULL))setFull(-1);
                if(jugada.contains(Constantes.JUGADA_POQUER))setPoquer(-1);
                if(jugada.contains(Constantes.JUGADA_GRANDE))setGrande(-1);
        }
    }
    ...
    public String listaDeJugadas(Cubilete cubilete) {
        ...
        if(cubilete.hayEscalera()&&getEscalera()==0)jugadas=jugadas+Constantes.JUGADA_ESCALERA+",";
        if(cubilete.hayFull()&&getFull()==0)jugadas=jugadas+Constantes.JUGADA_FULL+",";
        if(cubilete.hayPoquer()&&getPoquer()==0)jugadas=jugadas+Constantes.JUGADA_POQUER+",";
        if(cubilete.hayGrande()&&getGrande()==0)jugadas=jugadas+Constantes.JUGADA_GRANDE+",";
        
        if (jugadas.equals("")) {
            for (int i = 1; i <= 6; i++) {
                if(get(i)==0)jugadas=jugadas+"Borrar los "+i+",";
            }
            if(getEscalera()==0)jugadas=jugadas+"Borrar "+Constantes.JUGADA_ESCALERA+",";
            if(getFull()    ==0)jugadas=jugadas+"Borrar "+Constantes.JUGADA_FULL+",";
            if(getPoquer()  ==0)jugadas=jugadas+"Borrar "+Constantes.JUGADA_POQUER+",";
            if (getGrande() ==0)jugadas=jugadas+"Borrar "+Constantes.JUGADA_GRANDE+",";
        }
        return jugadas;
    }
}
